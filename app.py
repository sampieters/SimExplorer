...
from src.algorithm_manager import train_algorithms, predict_for_mock_user
...
# ==========================================
st.sidebar.markdown("---")
st.sidebar.header("2. Pipeline & Filters")

# Add New Filter UI
with st.sidebar.expander("Add Filter Step"):
...
if st.session_state.active_filters:
    ...
    for idx, f in enumerate(st.session_state.active_filters):
        ...
        if col_rem.button("❌", key=f"del_{idx}"):
            ...
            st.rerun()
        col_info.text(f"{f['name']} ({f['params']})")
...
selected_algos = st.sidebar.multiselect("Select Algorithms", ALGORITHMS, default=["ItemKNN (Cosine)"])
k_param = st.sidebar.slider("K (Neighbors)", 5, 100, 20)
    ...
    if st.session_state.filtered_df is None:
        ...
        with st.spinner("Training models..."):
            try:
                ...
                models = train_algorithms(matrix, selected_algos, k=k_param)
...
if st.session_state.filtered_df is not None:
    ...
    delta_shape = None
    if rm:
        d_users = -(rm.num_active_users - fm.num_active_users)
        d_items = -(rm.num_active_items - fm.num_active_items)
        delta_shape = (d_users, d_items)
        
    display_global_metrics(fm, "Processed Matrix", delta_shape)
...
if st.session_state.models:
    ...
    with st.expander("🔎 Similarity Explorer (Item-to-Item)", expanded=True):
    ...
    with st.expander("🕸️ Similarity Network Explorer"):
        col_s1, col_s2, col_s3, col_s4 = st.columns(4)
        sim_target = col_s1.selectbox("Target Model", list(st.session_state.models.keys()))
        sim_thresh = col_s2.slider("Min Similarity Threshold", 0.0, 1.0, 0.4)
        sim_deg = col_s3.number_input("Min Item Degree (Sim)", value=0, step=1)
        sim_max_n = col_s4.number_input("Max Nodes (Sim)", value=50, step=10)
        
        plot_similarity_graph(
            st.session_state.models[sim_target], 
            item_names=st.session_state.item_metadata, 
    ...
    for idx, (name, model) in enumerate(st.session_state.models.items()):
        with cols[idx]:
            st.subheader(name)
            fig = plot_similarity_distribution(model, name)
            if fig:
